...
// Per-request session context. The token is supplied by the caller (i.e. the
// browser) on every request — the server holds no state between requests.
// `rotated` captures any new ENEFITSESSION value the upstream issues during
// the call so the proxy can hand it back to the client to persist.
...
function captureRotation(session: Session, res: Response): void {
...
export async function getMeteringPoints(session: Session): Promise<MeteringPoint[]> {
  const data = (await getJson(`${BASE}/api/v1/metering-points/active`, session)) as unknown;
  const rows = extractArray(data);
  const out: MeteringPoint[] = [];
  for (const r of rows) {
