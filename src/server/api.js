import { EnefitError, getConsumption, getMeteringPoints, getPrices, pingSession, } from "./enefit";
...
async function handleUsage(session, start, end, point) {
    ...
    const consPerDay = await mapWithConcurrency(days, 4, (d) => getConsumption(session, point, d.startInclusive, d.endExclusive));
    const prices = await getPrices(session, start, end);
    const priceByHour = new Map();
    for (const p of prices) {
        if (p.hourKey)
            priceByHour.set(p.hourKey, p.eurPerKwh);
    }
    const intervals = [];
    let totalKwh = 0;
    let totalEur = 0;
...
export function enefitApiPlugin() {
    const middleware = async (req, res, next) => {
        ...
        try {
            ...
            if (url.pathname === "/api/session" && req.method === "GET") {
                ...
                res.statusCode = 200;
                res.end(JSON.stringify({ configured: true, ok: r.ok, upstream: r.status }));
                return;
            }
            if (!token) {
                res.statusCode = 401;
            ...
            if (url.pathname === "/api/usage") {
                ...
                if (!ymdRegex(start) || !ymdRegex(end) || end < start) {
                    res.statusCode = 400;
                    res.end(JSON.stringify({ error: "bad request", hint: "use start and end as YYYY-MM-DD" }));
                    return;
                }
                if (!point) {
                    res.statusCode = 400;
