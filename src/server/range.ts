...
function tallinnOffset(date: Date): string {
...
export interface DayRange {
  ...
  startInclusive: string; // ISO with offset
  endExclusive: string; // ISO with offset (23:59:59.999)
}
...
function ymdToUtcMidnight(y: number, m: number, d: number): Date {
...
export function eachDay(start: string, end: string): DayRange[] {
  const out: DayRange[] = [];
  let cur = start;
  while (cur <= end) {
